window.S_LOCALES = {};
window.S_LOCALES["en"] = { SEARCH: "Найти сотрудника", NO_RESULT: "Нет результатов", PLACEHOLDER_SINGLE: "Выберите сотрудника", PLACEHOLDER_MULTI: "Выберите сотрудников" };
    ...
    getOptions() {
        ...
        this.old.querySelectorAll(":scope > optgroup").forEach((op) => {
            op.querySelectorAll(":scope > option").forEach((ch) => {
                i++;
                res.push({ key: i, title: ch.innerText.trim(), value: ch.value, img: ch.dataset.img || "", desc: ch.dataset.desc || "", group: op.label, selected: ch.selected, disabled: ch.disabled });
            });
        });
        this.old.querySelectorAll(":scope > option").forEach((op) => {
            i++;
            res.push({ key: i, title: op.innerText.trim(), value: op.value, img: op.dataset.img || "", desc: op.dataset.desc || "", selected: op.selected, disabled: op.disabled });
        });
        return res;
    }
